...
void crearEquipos(Equipo arr[]){
    ...
    asignarJugadores(jugadores, 1,2,3,4);
    arr[5] = crearEquipo(1, id_partido, time(NULL), 10, jugadores);

    asignarJugadores(jugadores, 1,2,3,4);
    arr[6] = crearEquipo(1, id_partido, time(NULL), 10, jugadores);
    */
}
...
template <typename T>
Equipo *insertarOrdenado(Nodo<Equipo> *&lista, Equipo equipo, int (*criterio)(T, T)){
    Nodo<Equipo> *nuevo = new Nodo<Equipo>();
    nuevo->info = equipo;
    nuevo->sig = NULL;
    
    Nodo<Equipo> *aux = lista;
    Nodo<Equipo> *ant = NULL;
    
    while(aux != NULL && criterio(aux->info, equipo) <= 0){
        ant = aux;
        aux = aux->sig;
    }
...
void cargarHistorico(Nodo<Equipo> *equipos[], string nombreArchivo){
    ...
    fread(&equipo, sizeof(Equipo), 1, f);
    while(!feof(f)){
        insertarOrdenado(equipos[equipo.id_equipo], equipo, criterioIdEquipoAsc);
